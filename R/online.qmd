...
We will update this tutorial when necessary. Readers can access the latest version in our [GitHub repository](https://github.com/itchyshin/location-scale_meta-analysis).
...
visualize_fixed_effects <- function(model) {
  ...
  tryCatch({
    ...
    ggplot(fixed_effects_samples, aes(x = .value, y = .variable)) +
      ...
      geom_vline(xintercept = 0, linetype = "dashed", color = "grey33") +
...
`brms` is powerful Bayesian modelling package. While it is a package that is dedicated to meta-analysis, it can be used to fit meta-analysis models. An important part of using Bayesian approach is to choose proper priors. For the illustrative purpose, we will use the default priors that are designed to be weakly informative so that they provide moderate regularization and help stabilize computation. In your own analysis, you should choose more informative priors via, for example, prior elicitation. 
...
Given that it takes some time to run the `brms` model, you can download our pre-fitted `brms` model objects from: 
https://www.dropbox.com/scl/fo/tq2ogshquc5pa6zn6pr58/APG2gtCb6tSKgEnW3KgGJYY?rlkey=xrq6io33vu38txmtu71hpq4un&e=1&dl=0.
...
saveRDS(fit_ma9, here("Rdata", "ma9.rds"))
```
...
```{r}
fit_ma10 <- readRDS(here("Rdata", "fit_ma10.rds"))
print(fit_ma10)
```
...
There two potential mistakes when using `brms` to fit a location-scale meta-regression.

Using the data from **Example 1** as the example, the first mistake is to set `se(si)` and `sigma ~ 1 + habitat` at the same time (where `si` is the standard error of the observations and `habitat` is the moderator of the scale parameter `sigma`). This is because setting `se(si)` means the scale parameter `sigma` is replaced by `si` and `brms` will not estimate the scale part of the location-scale meta-regression.

It is easy to check it. Setting priors for such model structures will trigger the following error message `"Error: Cannot predict or fix 'sigma' in this model."`

```{r}
#| eval: false
form1 <- bf(dARR | se(si) ~ 1 + habitat + (1|study_ID), # this is u
            sigma ~ 1 + habitat)
...
The second potential mistake is to set `se(si, sigma = TRUE)` and `sigma ~ 1 + habitat`. This is because such a specification would assume that the sampling variance is not perfectly known, but only up to a constant of proportionality (the so-called residual variance).

Ask Shinichi: actually, I am wondering whether this is really a mistake because I remember that only weighted regression with inverse-sampling-variances as weights will assume a multiplicative heterogeneity.

```{r}
#| eval: false
# this is wrong too - as it multiple sigma^2 to sampling variance 
# (in meta-analysis, sampling variance is assumed to be known)
...
- Because location-scale model implemented in `brm()` is designed for general linear (mixed effects) regression framework, so we have to hack `brm()` to make it work for location-scale meta-analysis. The meta-analytic models are just a special case of a general linear (mixed effects) model, with one usual aspect that the variance of the error term (known as the sampling variance) is known. Therefore, to make `brm()` suitable for fitting a location scale meta-analytical model, we need to fix the `sigma` parameter (residual or error term) using the known sampling errors whilst making the `sigma` parameter is not fixed so that it could be estimated and modeled. 
While it sounds counterintuitive and in-feasible, it can be achieved by the trick of `(1|gr(es_ID, cov = vcv))`. The intuition behind it is that we the known sampling errors to use the within-study-random-effect's placeholder so that sampling errors are accounted for but the within-study random effect needs to be estimated. Specifically, `(1 | ...)` defines a random intercept for each level of of observation (`es_ID`). `gr(es_ID, cov = vcv)` specify a pre-defined variance-covariance matrix `vcv` to account for the known sampling errors.
...
Again, given that it takes some time to run the `brms` model, you can download our pre-fitted `brms` model objects from: https://www.dropbox.com/scl/fo/tq2ogshquc5pa6zn6pr58/APG2gtCb6tSKgEnW3KgGJYY?rlkey= xrq6io33vu38txmtu71hpq4un&e=1&dl=0.
...
We also propose mean-standardized heterogeneity measure in our paper. They also can be computed from `brms` object.
...
One key step of accounting correlated sampling errors is to construct a variance-covariance matrix. `vcalc()` from `metafor` allows us easily do so. 
...
Looking at `Regression Coefficients`, We can see `terrestrial` has a lower effect than `aquatic` (`-0.16, 95% CI: −0.23 to −0.29`). Meanwhile `terrestrial` is also less variable than `aquatic` (`-1.18, 95% CI: −1.33 to −1.02`). If we use traditional meta-analytic approaches (which assume a fixed variance), we will lose finding.
...
We see that a relatively large phylogenetic signal (0.94). Visualization also confirms that there is a large phylogenetic variance:
...
**Example 2** involves using a dataset (`elevation.csv`) to meta-analyse the impact of elevation on intraspecific leaf trait. Let's load `elevation.csv` into `R`, and do some basic pre-processes:
...
This data were originally used to study the effect of forest disturbance on pollination, seed dispersal, seed predation, recruitment and herbivore during plant regeneration. Now, we will it to illustrate the proposed four types of publication bias, including small-study effect, decline effect, small-study divergence, and Proteus effect. While the first two (Equations (35)) are relevant to the location part of the location-scale meta-analysis, the last two (Equation (36)) are relevant to the scale part of the location-scale meta-analysis.
...
However, the` Rhat` values are far from 1, signifying model convergence issues. The effective sample sizes of the bulk and tails of the MCMC are extremely small. These are indications that the parameters are not reliably estimated.

Let's do more tests to confirm this. The first test we can do is the posterior predictive test. This test will simulate a random sample from the posterior predictive distribution and then compare the simulated sample (replication) to the observed effect size. If the `fit_ma3` model has converged and captured the data well, we can expect the density of replications to be roughly similar to that of the observed data. This check indicates that the replications deviate from the observed effect size:

```{r}
pp_check(fit_ma3)
```

Next, we can display each chain. The result clearly show that one chain does not converge:
...
The posterior predictive test still does not look good:
...
Let's further remove another parameter from the scale part, namely the between-study effect:

TODO - Shinichi, could you please help run this `brms` model (taking out `(1|study_ID)`  from `sigma ~ 1 + (1|study_ID)`). While this will collapse to a normal meta-analysis, it uses a different framework and will be a good illustration why we recommend removing random effects from the scale part when fitting location-scale regression.
...
Ask Shinichi: `|p|` should be removed and `(1|p|study_ID)` in scale part should also be removed
...
Ask Shinichi: not sure we will have the following code in this tutorial and whether we should use the following figures in the main text. Because the results from `metafor` which ignores the scale part are distinct from those from `brms` and `blsmeta`.
...
# issues in the main text:

1. sigma should be sigma2 (earlier section)

2. sigma2 for the I2 should be sigma2_l (location part)

3. example section: remove significantly, add mention software, and `lsmeta` and `metafor` show similar results

4. variance of the random effects in scale part is not the difference and should not be interpreted as the heterocedasticity (which should be difference in variance)

5. Figure 1 - sampling effort? Figure 4 - the figure indicates there is small-study effect, but the legend says no.

6. subtitle of 2.2 - should it be random-effects location-scale meta-regression