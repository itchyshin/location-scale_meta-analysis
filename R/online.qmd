...
rename_vars <- function(variable) {
  variable <- gsub("b_Intercept", "b0_l", variable)
   variable <- gsub("b_sigma_Intercept", "b0_s", variable)  
...
In reality, non-indepedence of sampling errors are common (effect sizes are obtained partially or all from the same subjects or individuals)
...
Not that there is non-zero variance (SD) in the between-study effect on the scale part as well as the location part. Also, there is a positive and significant correaltion ($\rho_u = 0.38$) between the location and scale between-study random effects.
