...
This online material is a supplement to the paper about multilevel location-scale meta-analysis and meta-regression.
The main aim of this online material is to provide a comprehensive hands-on guide for the implementation of location-scale meta-analysis and meta-regression.
While we try to make this online material as stand-along as possible (see below), we do not have too much explanation of the theories underpinning the location-scale meta-analysis and meta-regression.
Therefore, we recommend readers to read the associated paper for theoretical background, formulas, and details on location-scale models in ecology and evolution.
...
Given that it takes some time to run the `brms` model, you can download our pre-fitted `brms` model objects from: https://www.dropbox.com/scl/fo/tq2ogshquc5pa6zn6pr58/APG2gtCb6tSKgEnW3KgGJYY?r
...
Using the data from **Example 1** as the example, the first mistake is to set `se(si)` and `sigma ~ 1 + habitat` at the same time (where `si` is the standard error of the observations and `habitat` is the moderator of the scale parameter `sigma`).
This is because setting `se(si)` means the scale parameter `sigma` is replaced by `si` and `brms` will not estimate the scale part of the location-scale meta-regression.
...
The second potential mistake is to set `se(si, sigma = TRUE)` and `sigma ~ 1 + habitat`.
This is because such a specification would assume that the sampling variance is not perfectly known, but only up to a constant of proportionality (the so-called residual variance).
...
Let's do more tests to confirm this.
The first test we can do is the posterior predictive test.
This test will simulate a random sample from the posterior predictive distribution and then compare the simulated sample (replication) to the observed effect size.
If the `fit_ma3` model has converged and captured the data well, we can expect the density of replications to be roughly similar to that of the observed data.
This check indicates that the replications deviate from the observed effect size:

```{r}
pp_check(fit_ma3)
```

Next, we can display each chain.
...
TODO - Shinichi, could you please help run this `brms` model (taking out `(1|study_ID)` from `sigma ~ 1 + (1|study_ID)`).
While this will collapse to a normal meta-analysis, it uses a different framework and will be a good illustration why we recommend removing random effects from the scale part when fitting location-scale regression.
