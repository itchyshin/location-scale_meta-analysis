...
dat <- dat %>% select(dARR, Var_dARR, es_ID, population_ID, study_ID, exp_design, habitat, phylogeny, species_ID)

# creating SE (sqrt(Var))
dat$si <- sqrt(dat$Var_dARR)

# mending some species names in phylogeny

dat$phylogeny[dat$phylogeny == "Oncorhynchus_mykiss_(species_in_domain_Eukaryota)"] <- 
"Oncorhynchus_mykiss_-species_in_domain_Eukaryota"
...
Yet this is not a probably location-scale meta-regression. Symbolically, this model can be written as

$$
y_i = \beta_0^{(l)} + \beta_1^{(l)} x_{1i} + u_{j[i]}^{(l)} + e_i^{(l)},
$$
$$
...
$$
u_j^{(l)} \sim \mathcal{N}(0,\sigma_{u(l)}^{2}) \space \text{and} \space \space e_i \sim \mathcal{N}(0,\sigma_e^2 \sigma_{m_i}^2), 
$$
As you see above, the sampling variance is merged with the residual variance (the within-study variance) and this is not a location-scale model, which we define in the main text
...
saveRDS(fit_mr2, here("Rdata", "fit1b.rds"))
...
```{r}
# read in the tree
tree <- read.tree(here("data", "phylo_tree.tre")) # Read in the tree
...
### Visualzing phylogenetic meta-regression results (biological)
...
The parameters are not mixing well and we have convergence issues which are also clear from the fig below. 
...
![Fig 4](/Users/itchyshin/Dropbox/Github Local/location-scale_meta-analysis/fig/Fig_examples.png)
