...
Ask Shinichi: not sure we will have the following code in this tutorial and whether we should use the following figures in the main text. Because the results from `metafor` which ignores the scale part are distinct from those from `brms` and `lsmeta`.
