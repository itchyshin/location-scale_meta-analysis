...
write.tree(phylo_branched, file = here("data", "phylo_tree.tre"))# Save the tree to a file
...
design_dat<-d.tree %>% group_by(tip.label) %>% summarise( 
                                               initial=brought_common_temp=="no", 
                                               persistent=brought_common_temp=="yes",
                                               CTmax=metric=="CTmax",
                                               LT50=metric=="LT50") # Summarise, for each species, whether they tested initial effects, persistent effects, CTmax, or LT50
...
design_dat<-design_dat %>% group_by(tip.label) %>% summarise(initial=sum(initial),        
...
design_dat <- mutate(design_dat, 
                     persistence = 
                          ifelse(initial=="1"&persistent=="1", "both",
                          ifelse(initial=="1"&persistent=="0", "initial", "persistent"))) # If both initial and persistent, indicate "both", if only initial, indicate "initial", otherwise indicate "persistent"
...
p3<- p2 + new_scale_fill()+ geom_fruit(geom=geom_tile, mapping=aes(fill=persistence),offset=0.1,width=0.07, col="gray30")+scale_fill_manual(values=c("#D95F02","#7570B3","#1B9E77")) # Create tiles to indicate whether initial or persistent effects were assessed
...
save(VCV_dARR, 
     phylo_matrix,
     file=here("RData","Var_and_phylo_matrices.RData"))
...
plot_continuous(d.common_temp, mod.common_temp_log, d.common_temp$time_common_temp, "Time at common temperature (days)")
```
...
save(mod.habitat,  # Habitat without intercept
     ...
     mod.persist_cont, # Contrast between initial and persistent effects
     ...
     mod.persist_het_cont, # Contrasts between initial and persistent effects, with heteroscedasticity modeled
...
## *Analyses with initial designs only* {.tabset .tabset_fade .tabset_pills}
...
my.orchard(mod.initial_habitat, xlab="dARR", mod="habitat", alpha=0.1, data=d.initial, group="species_ID", whisker=0.07)
```
...
```{r}
VCV_initial<-make_VCV_matrix(d.initial, V="Var_dARR", cluster="shared_trt_ID", obs="es_ID")
mod.initial_habitat_het <-  rma.mv(yi=dARR, 
                           V=VCV_initial, 
                           mods= ~habitat-1,
                           method="REML",
                           test="t",
...
my.orchard(mod.initial_habitat_het, mod="habitat", xlab="dARR", alpha=0.1, data=d.initial, group="species_ID", whisker=0.07) 
```
...
```{r, fig.width=12, fig.height=8}
mod.adjusted_initial<-mod_results(model=mod.initial_habitat_het, data=d.initial, mod="1", weights="equal", group="species_ID")
mod.adjusted_initial$mod_table

my.orchard(mod.adjusted_initial, mod="1", xlab="dARR", alpha=0.1, data=d.initial, group="species_ID", whisker=0.04)
```
...
my.orchard(mod.initial_taxa, xlab="dARR", mod="taxonomic_group", data=d.initial, alpha=0.1, group="species_ID", whisker=0.09)
```

#### Heteroscedasticity check 

```{r, fig.height=6, fig.width=12}
qplot(y = sqrt(residuals(mod.initial_taxa)^2), x = fitted(mod.initial_taxa)) + 
      geom_point() +
      geom_smooth(method = "lm") + 
      geom_hline(yintercept = 0, colour="red") 
```
...
```{r, fig.height=10, fig.width=12}
mod.initial_taxa_het <-  rma.mv(yi=dARR, 
                           V=VCV_initial, 
                           mods= ~taxonomic_group-1,
                           method="REML",
                           test="t",
...
my.orchard(mod.initial_taxa_het, mod="taxonomic_group", xlab="dARR", alpha=0.1, data=d.initial, group="species_ID", whisker=0.09) 
```
...
my.orchard(mod.initial_metric, xlab="dARR", mod="metric", alpha=0.1, data=d.initial, group="species_ID", whisker=0.07)
```
...
```{r, fig.height=10, fig.width=12}
mod.initial_metric_het <-  rma.mv(yi=dARR, 
                           V=VCV_initial, 
                           mods= ~metric-1,
                           method="REML",
                           test="t",
...
my.orchard(mod.initial_metric_het, mod="metric", xlab="dARR", alpha=0.1, data=d.initial, group="species_ID", whisker=0.07) 
```
...
plot_continuous(d.initial_ramping, mod.initial_ramping, d.initial_ramping$ramping, "Heating rate (degrees/min)")
```

```{r, eval=F, include=F}
save(mod.initial_habitat, 
     mod.initial_habitat_het,
     mod.adjusted_initial,
     mod.initial_taxa, 
     mod.initial_taxa_het,
     mod.initial_metric, 
     mod.initial_metric_het,
     mod.initial_ramping, 
     
     file=here("RData","Sens_initial_only.RData"))

load(here("Rdata", "Sens_initial_only.RData")) 
```
...
my.orchard(mod.imputed, xlab="dARR", mod="1", alpha=0.1, data=filter(d, imputed=="no"), group="species_ID", whisker=0.07)
```
...
my.orchard(mod.concern, xlab="dARR", mod="1", alpha=0.1, data=filter(d, is_concern=="no"), group="species_ID", whisker=0.07)
```
...
my.orchard(mod.extreme1.25, xlab="dARR", mod="1", alpha=0.1, data=filter(d, dARR>-1.25&dARR<1.25), group="species_ID", whisker=0.05)
...
my.orchard(mod.extreme0.5, xlab="dARR", mod="1", alpha=0.1, data=filter(d, dARR>-0.5), group="species_ID", whisker=0.05)
...
my.orchard(mod.extreme0.15, xlab="dARR", mod="1", alpha=0.1, data=filter(d, dARR>-0.15), group="species_ID", whisker=0.05)
...
plot_continuous(d.mass.initial, mod.body_mass.initial, log(d.mass.initial$body_mass), "ln body mass (g)")
```
...
plot_continuous(d.mass.persistent, mod.body_mass.persistent, log(d.mass.persistent$body_mass), "ln body mass (g)")
```
...
plot_continuous(d.maturity.initial, mod.maturity.initial, log(d.maturity.initial$age_maturity), "ln age at sexual maturity (years)")
```
...
plot_continuous(d.maturity.persistent, mod.maturity.persistent, log(d.maturity.persistent$age_maturity), "ln age at sexual maturity (years)")
```
...
```{r, fig.width=12, fig.height=8}
d<-d %>% mutate(ratio_reacc_maturity = (time_common_temp / (age_maturity*365.25)))

mod.common_temp_maturity<- run.model(d, ~scale(log(ratio_reacc_maturity), center=T, scale=F)) # Center to zero mean
...
plot_continuous(d.reacc.maturity, mod.common_temp_maturity, d.reacc.maturity$ratio_reacc_maturity, "Re-acclimation time / Age at sexual maturity")
```

### Life history variation after accounting for different re-acclimation times 

```{r, fig.width=12, fig.height=8}
mod.common_temp_design <- run.model(d, ~scale(log(time_common_temp), center=T, scale=F)+exp_design)
summary(mod.common_temp_design)

mod.common_temp_maturity_design <- run.model(d, ~scale(log(ratio_reacc_maturity), center=T, scale=F)+exp_design)
...
```{r}
mod.lat_mass<- run.model(d, ~scale(log(body_mass))*
                             scale(abs(latitude)))
...
```{r}
mod.lat_ramp<- run.model(d, ~scale(abs(latitude))*
                             scale(log(ramping)))
...
```{r}
mod.lat_time<- run.model(d, ~scale(abs(latitude))*
...
```{r}
mod.mass_ramp<- run.model(d, ~scale(log(body_mass))*
                              scale(log(ramping)))
...
```{r}
mod.mass_time<- run.model(d, ~scale(log(body_mass))*
                              scale(log(acc_duration)))
...
```{r}
mod.ramp_time<- run.model(d, ~scale(log(ramping))*
                              scale(log(acc_duration)))
...
```{r}
mod.lat_mass_ramp<- run.model(d, ~scale(log(body_mass))*
                                  scale(log(ramping))*
                                  scale(abs(latitude)))
...
```{r}
mod.lat_mass_time<- run.model(d, ~scale(log(body_mass))*
                                  scale(log(acc_duration))*
                                  scale(abs(latitude)))
...
```{r}
mod.lat_ramp_time<- run.model(d, ~scale(log(acc_duration))*
                                  scale(log(ramping))*
                                  scale(abs(latitude)))
...
```{r}
mod.mass_ramp_time<- run.model(d, ~scale(log(body_mass))*
                                  scale(log(ramping))*
                                  scale(log(acc_duration)))
...
```{r}
mod.mass_ramp_time_habitat<- run.model(d, ~scale(log(body_mass))*
                                  scale(log(ramping))*
...
```{r}
mod.mass_ramp_time_taxa<- run.model(d, ~scale(log(body_mass))*
                                  scale(log(ramping))*
...
```{r}
mod.mass_ramp_time_design<- run.model(d, ~scale(log(body_mass))*
                                  scale(log(ramping))*
...
#### Latitude x Body mass
```{r}
mod.lat_mass_initial<- run.model(d.initial, ~scale(log(body_mass))*
                                            scale(abs(latitude)))
...
#### Latitude x Heating rate
```{r}
mod.lat_ramp_initial<- run.model(d.initial, ~scale(abs(latitude))*
                                            scale(log(ramping)))
...
#### Latitude x Acclimation duration
```{r}
mod.lat_time_initial<- run.model(d.initial, ~scale(abs(latitude))*
                                             scale(log(acc_duration)))
...
#### Body mass x Heating rate
```{r}
mod.mass_ramp_initial<- run.model(d.initial, ~scale(log(body_mass))*
                                              scale(log(ramping)))
...
#### Body mass x Acclimation duration
```{r}
mod.mass_time_initial<- run.model(d.initial, ~scale(log(body_mass))*
                              scale(log(acc_duration)))
...
#### Heating rate x Acclimation duration
```{r}
mod.ramp_time_initial<- run.model(d.initial, ~scale(log(ramping))*
                              scale(log(acc_duration)))
...
#### Latitude x Body mass x Heating rate
```{r}
mod.lat_mass_ramp_initial<- run.model(d.initial, ~scale(log(body_mass))*
                                                  scale(log(ramping))*
                                                  scale(abs(latitude)))
...
#### Latitude x Body mass x Acclimation duration
```{r}
mod.lat_mass_time_initial<- run.model(d.initial, ~scale(log(body_mass))*
                                  scale(log(acc_duration))*
                                  scale(abs(latitude)))
...
#### Latitude x Heating rate x Acclimation duration
```{r}
mod.lat_ramp_time_initial<- run.model(d.initial, ~scale(log(acc_duration))*
                                  scale(log(ramping))*
                                  scale(abs(latitude)))
...
#### Body size x Heating rate x Acclimation duration
```{r}
mod.mass_ramp_time_initial<- run.model(d.initial, ~scale(log(body_mass))*
                                  scale(log(ramping))*
                                  scale(log(acc_duration)))
...
#### Latitude x Body mass
```{r}
mod.lat_mass_persistent<- run.model(d.persistent, ~scale(log(body_mass))*
                                                  scale(abs(latitude)))
...
#### Latitude x Heating rate
```{r}
mod.lat_ramp_persistent<- run.model(d.persistent, ~scale(abs(latitude))*
                                                   scale(log(ramping)))
...
#### Latitude x Acclimation duration
```{r}
mod.lat_time_persistent<- run.model(d.persistent, ~scale(abs(latitude))*
                                                  scale(log(acc_duration)))
...
#### Body mass x Heating rate
```{r}
mod.mass_ramp_persistent<- run.model(d.persistent, ~scale(log(body_mass))*
                                                    scale(log(ramping)))
...
#### Body mass x Acclimation duration
```{r}
mod.mass_time_persistent<- run.model(d.persistent, ~scale(log(body_mass))*
                              scale(log(acc_duration)))
...
#### Heating rate x Acclimation duration
```{r}
mod.ramp_time_persistent<- run.model(d.persistent, ~scale(log(ramping))*
                              scale(log(acc_duration)))
...
```{r}
d.crit<-d %>% mutate(d, Tolerance_landscape=ifelse(major_concerns=="The CTmax was estimated from death times at different temperatures (thermal tolerance landscapes)."|major_concerns=="The CTmax was estimated from death times at different temperatures (thermal tolerance landscapes).\\", "yes", "no"))
d.crit<- filter(d.crit, Tolerance_landscape!="yes"|is.na(Tolerance_landscape)=="TRUE")                    
...
```{r, include=F, eval=F}
save(mod.body_mass,
     mod.body_mass.initial,
     mod.body_mass.persistent,
     mod.maturity,
     mod.maturity.initial,
     mod.maturity.persistent, 
     
     file=here("RData","Post-hoc_models.RData"))
```
...
p3<- p2 + new_scale_fill()+ geom_fruit(geom=geom_tile, mapping=aes(fill=initial_persistent),offset=0.1,width=0.07, col="black", size=0.3)+scale_fill_manual(values=c("#EAD295","#9994EA","#95EA99"), name="Design")+theme(legend.direction = "vertical")
